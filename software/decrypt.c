#include <stdio.h>

int MD5_Init(char *);
int MD5_Update(char *, char *, int);
int MD5_Final(char *, char *);

unsigned char crc32[] = {};

unsigned char input[2424];

unsigned int l32(unsigned char input[], unsigned int offset) {
	unsigned int tmp;
	tmp = input[offset + 3] << 24;
	tmp = tmp + (input[offset + 2] << 16);
	tmp = tmp + (input[offset + 1] << 8);
	tmp = tmp + input[offset];
	return tmp;
}

unsigned int t16(unsigned int start, unsigned int num) {
	unsigned int crc = 0xffffffff;
	unsigned int i;
	unsigned int ip;
	for(i=start; i < (start+num); i++) {
		ip = (input[i] ^ crc); //& low32
		ip = (ip & 255); //& low32
		ip = l32(crc32, (ip << 2));
		crc = (crc >> 8) ^ ip; //# & low32 # eor	r2, ip, r2, lsr #8
	}
	return ~crc;
}

int main(int argc, char *argv[]) {
	printf("XM decryption tool\n");
	unsigned int s;
	unsigned int i;
	FILE *fp;
	unsigned char res;
	unsigned char mac[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

	//fp=fopen("./encrypted.bin", "r");
	fp=fopen(argv[1], "r");

	s = fgetc(fp);
	i = 0;
	while (i < 1024) {
		//printf("0x%x ", s);
		input[i] = (unsigned char)s;
		input[i + 0x400] = (unsigned char)s;
		i++;
		s = fgetc(fp);
	}
	fclose(fp);
	printf("File readed (first 1024 bytes).\n");

	//Step 1: checl total crc
	i = t16(0, 1020);
	printf("Calculated CRC = 0x%x\n", i);
	s = l32(input, 1020);
	printf("Data CRC = 0x%x\n", s);
	if (i != s) {
		printf("Step 1 CRC error!\n");
		return 1;
	}
	printf("Step 1 CRC OK.\n");

	//Step 2: swap MAC bytes
	for(i=0; i<6; i++) {
		s = input[1024 + 28 + i];
		input[1024 + 28 + i] = input[1024 + 889 + (i*2)];
		input[2416 + i] = input[1024 + 889 + (i*2)];
		input[1024 + 889 + (i*2)] = s;	
	}
	printf("Step 2 bytes swaped\n");

	//Step 3, check data blocks
	//1st block
	input[0x457] = input[0x457] - 1; 
	MD5_Init(&input[0x900]); 
	MD5_Update(&input[0x900], &input[0x418], 0x40);
	MD5_Final(&input[0x968], &input[0x900]);
	res = memcmp (&input[0x968], &input[0x458], 8);
	if (res != 0) {
		printf("Step 3 block 1 md5 error!\n", res);
		return 1;
	}
	printf("Step 3 block 1 md5 OK.\n", res);


	//2nd block
	input[0x576] = input[0x576] - 1; 
	i = t16(0x538, 0x40);
	s = l32(input, 0x578); //find stored crc -> [0x538+67][0x538+66][0x538+65][0x538+64]
	if (i != s ) {
		printf("Step 3 block 2 crc error!\n");
		return 1;
	}
	printf("Step 3 block 2 crc OK.\n");


	//3rd block
	input[0x695] = input[0x695] - 1;
	i = t16(0x658, 0x40);
	s = l32(input, 0x698);
	if (i != s ) {
		printf("Step 3 block 3 crc error!\n");
		return 1;
	}
	printf("Step 3 block 3 crc OK.\n");

	//4th block
	input[0x7b4] = input[0x7b4] - 1;
	MD5_Init(&input[0x900]);
	MD5_Update(&input[0x900], &input[0x778], 0x40);
	MD5_Final(&input[0x968], &input[0x900]);
	res = memcmp (&input[0x968], &input[0x7b8], 8);
	if (res != 0) {
		printf("Step 3 block 4 md5 error!\n", res);
		return 1;
	}
	printf("Step 3 block 4 md5 OK.\n", res);

	//Step 4 XOR
	//1st block
	for (i = 0; i<64; i++) {
		if ((input[0x418+i]^0xa5) != input[0x498+i]) {
			printf("Step 4 block 1 XOR error, byte %d!\n", i);
			return 1;
		}
	}	
	printf("Step 4 block 1 XOR OK.\n");

	//2nd block
	for (i = 0; i<64; i++) {
		if ((input[0x538+i]^0xa5) != input[0x5b8+i]) {
			printf("Step 4 block 2 XOR error, byte %d!\n", i);
			return 1;
		}
	}	
	printf("Step 4 block 2 XOR OK.\n");

	//3rd block
	for (i = 0; i<64; i++) {
		if ((input[0x658+i]^0xa5) != input[0x6d8+i]) {
			printf("Step 4 block 3 XOR error, byte %d!\n", i);
			return 1;
		}
	}	
	printf("Step 4 block 3 XOR OK.\n");

	for (i=0;i<8;i++) {
		if ((input[0x778 + i]^0xa5) != input[0x7ee + i]) {
			printf("Step 4 block 4 XOR error, byte %d!\n", i);
			return 1;
		}
	}
	printf("Step 4 block 4 XOR OK.\n");

	/*
	s = 0;
	for(i=1024;i<2424;i++) {
		if (s == 8) { printf("\n"); s = 0; }
		printf("0x%x\t", input[i]);
		s++;
	}
	printf("\n");
	*/

	//Get MAC
	mac[0] = input[0x41c + 0] - 1;
	mac[1] = input[0x41c + 1] - 3;
	mac[2] = input[0x41c + 2] - 5;
	mac[3] = input[0x41c + 3] - 7;
	mac[4] = input[0x41c + 4] - 9;
	mac[5] = input[0x41c + 5] - 11;

	printf("MAC: %02x:%02x:%02x:%02x:%02x:%02x.\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	return 0;
}
/*
==============MEMORY MAP===============
000 * * * * * * * *	* * * * * * * *
010 * * * * * * * *    [d d d d M M M M
020 M M d d d d d d	d d d d d d d d
030 d d d d d d d d	d d d d d d d d
040 d d d d d d d d	d d d d d d d d
050 d d d d d d d d]	MD5 hash block1
060 * * * * * * * *	* * * * * * * *
070 * * * * * * * *	* * * * * * * *
080 * * * * * * * *	* * * * * * * *
090 * * * * * * * *	X X X X X X X X
0a0 X X X X X X X X	X X X X X X X X
0b0 X X X X X X X X	X X X X X X X X
0c0 X X X X X X X X	X X X X X X X X
0d0 X X X X X X X X	* * * * * * * *
0e0 * * * * * * * *	* * * * * * * *
0f0 * * * * * * * *	* * * * * * * *
100 * * * * * * * *	* * * * * * * *
110 * * * * * * * *	* * * * * * * *
120 * * * * * * * *	* * * * * * * *
130 * * * * * * * *    [d d d d d d d d
140 d d d d d d d d	d d d d d d d d
150 d d d d d d d d	d d d d d d d d
160 d d d d d d d d	d d d d d d d d
170 d d d d d d d d]	CRC T16 * * * *
180 * * * * * * * *	* * * * * * * *
190 * * * * * * * *	* * * * * * * *
1a0 * * * * * * * *	* * * * * * * *
1b0 * * * * * * * *	X X X X X X X X
1c0 X X X X X X X X	X X X X X X X X
1d0 X X X X X X X X	X X X X X X X X
1e0 X X X X X X X X	X X X X X X X X
1f0 X X X X X X X X	* * * * * * * *
200 * * * * * * * *	* * * * * * * *
210 * * * * * * * *	* * * * * * * *
220 * * * * * * * *	* * * * * * * *
230 * * * * * * * *	* * * * * * * *
240 * * * * * * * *	* * * * * * * *
250 * * * * * * * *    [d d d d d d d d
260 d d d d d d d d	d d d d d d d d
270 d d d d d d d d	d d d d d d d d
280 d d d d d d d d	d d d d d d d d
290 d d d d d d d d]	CRC T16 * * * *
2a0 * * * * * * * *	* * * * * * * *
2b0 * * * * * * * *	* * * * * * * *
2c0 * * * * * * * *	* * * * * * * *
2d0 * * * * * * * *	X X X X X X X X
2e0 X X X X X X X X	X X X X X X X X
2f0 X X X X X X X X	X X X X X X X X
300 X X X X X X X X	X X X X X X X X
310 X X X X X X X X	* * * * * * * *
320 * * * * * * * *	* * * * * * * *
330 * * * * * * * *	* * * * * * * *
340 * * * * * * * *	* * * * * * * *
350 * * * * * * * *	* * * * * * * *
360 * * * * * * * *	* * * * * * * *
370 * * * * * * * *    [d m d m d m d m
380 d m d m d d d d	d d d d d d d d
390 d d d d d d d d	d d d d d d d d
3a0 d d d d d d d d	d d d d d d d d
3b0 d d d d d d d d]	MD5 hash block4
3c0 * * * * * * * *	* * * * * * * *
3d0 * * * * * * * *	* * * * * * * *
3e0 * * * * * * * *	* * * * * * * *
3f0 * * * * * * * *	* * * * CRC T16
*/
